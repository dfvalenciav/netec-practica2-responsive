class AllowedSwipeDirection { 
  ...
  const AllowedSwipeDirection.all() : right = true, left = true; 
  /// Does not allow the card to be swiped in any direction 
  const AllowedSwipeDirection.none() : right = false, left = false; 
  /// Allow the card to be swiped in only the specified directions 
  const AllowedSwipeDirection.only({ this.left = false, this.right = false, }); 
